...
export default function ClaimPage() {
  ...
  async function startSession() {
    try {
      // App API key isn't known publicly here; the claim page needs the
      // app's public claim identifier. We look it up via a lightweight
      // endpoint keyed by app id (safe: returns only name + task list,
      // never the api_key used for Roblox verification).
      ...
      setTasks(data.tasks);
      pushLog(`[i] session started for ${data.appName}`);
      setPhase('tasks');
    } catch (err) {
